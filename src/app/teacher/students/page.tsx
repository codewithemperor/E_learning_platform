...
export default function StudentsPage() {
  ...
  const fetchSubjects = async () => {
    if (!user) return;
    try {
      const response = await fetch(`/api/teacher/subjects?teacherId=${user.id}`);
      if (response.ok) {
  ...
  const fetchStudents = async (subjectId?: string) => {
    ...
    try {
      const url = subjectId 
        ? `/api/teacher/students?teacherId=${user.id}&subjectId=${subjectId}`
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="mt-2 space-y-1 text-sm">
                        <p><span className="font-medium">Name:</span> {selectedStudent.user.name}</p>
                        <p><span className="font-medium">Student ID:</span> {selectedStudent.studentId}</p>
                        <p><span className="font-medium">Email:</span> {selectedStudent.user.email}</p>
                      </div>
                    </div>
